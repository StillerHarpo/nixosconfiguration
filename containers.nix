...
with builtins;
let
  containerConf = foldl' (init: container:
    with container;
    let
  ...
  '';
    in
      {
        packages = init.packages ++ [ run ];
        allowedIpRanges =  [ "192.168.${addresses}.0/24"] ++ init.allowedIpRanges;
...
in with containerConf; {
  environment.systemPackages = packages ++ [ apulse ];
  inherit containers;
  hardware.pulseaudio.tcp.anonymousClients.allowedIpRanges = allowedIpRanges;
  networking = {
