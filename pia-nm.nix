...
let
  pia_user = readFile ./pia-user.conf;
  pia_pass = readFile ./pia-pass.conf;
...
in
{
  ...
  environment.etc = foldl' (init: name_uuid:
    let
      ...
      # autoconnect = if init == {} then "true" else "false";
    in
      init // {
      ...
      ) // { "NetworkManager/system-connections/global" = {
               text = ''
                 [connection]
                 match-device=type:wifi
                 autoconnect-slaves=1
                 secondaries=453bcca9-2fa9-4335-95ba-cf48dc9fd5da;
               '';
               mode = "600";
             };
           }
